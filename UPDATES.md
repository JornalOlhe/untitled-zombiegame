...
- v20: build QA com entradas de chefes estilo anime (super salto, sem fogo/neve nas costas), nova morte do Demônio (mãos do submundo o arrastam para baixo) e do Yeti (tenta fugir da bola de neve), morte nova do rastejante (estica no chão; no headshot a cabeça voa), medkits em pontos e tempos aleatórios, caixa de granadas a cada 1 minuto e texturas reais com normal maps nos mapas; Android usa `versionCode 20` / `0.20.0` e Windows usa `0.20.0`.
- v21: Windows passa a usar instalador (`DeadRecoil-Setup-<versão>.exe`) com atualização automática no lugar (baixa para uma pasta temporária fixa, confere o SHA-256 do GitHub, instala em silêncio e reabre o jogo; progresso em `%APPDATA%\Dead Recoil` é mantido). Android baixa o APK dentro do app e instala por cima; APK assinado com a chave de release fixa. Login Google no Windows por redirecionamento local (127.0.0.1) e e-mails pelo SMTP do Brevo; Android usa `versionCode 21` / `0.21.0` e Windows usa `0.21.0`.

- v22: recuperação de senha por código de 8 números enviado por e-mail (funciona em qualquer aparelho), entregue pela atualização automática da v21; Android usa `versionCode 22` / `0.22.0` e Windows usa `0.22.0`.
No Windows, a v10 compara a versão do executável com as tags `vN` publicadas. Quando uma tag mais nova como `v20` estiver disponível, o jogo informa que a versão instalada está desatualizada e oferece o download do asset `DeadRecoil-Windows.exe`.
